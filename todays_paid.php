...
$sql = "SELECT p.payment_id, p.amount, p.balance,p.payment_date, p.payment_type, p.payment_status, 
               c.customer_name as customer_name, c.phone_number as phone_number, c.stb_id, c.lco_id 
        FROM payments p 
        JOIN customers c ON p.customer_id = c.customer_id 
        WHERE p.payment_date = CURDATE() AND p.payment_status = 'paid'";
    ...
    var table = $('#paymentsTable').DataTable({
        ...
        "footerCallback": function ( row, data, start, end, display ) {
            ...
            document.getElementById('export-btn').addEventListener('click', function() {
                ...
                XLSX.writeFile(wb, 'todays_payments.xlsx');
            });
        });
    </script>
</body>
